...
const ProjectsList: React.FC<ProjectsListProps> = ({ projects, showCreateProjectCard = true, enableDelete = true, onProjectClick }) => {
  return (
    <ul className="flex flex-col list-none h-full overflow-y-auto">
      {projects && projects.length > 0 ? (
        <>
          {projects.map((project) => (
            <li key={project.id} className="mb-4 pr-10 pl-5 pt-5" onClick={() => onProjectClick(project)}>
      ...
      {showCreateProjectCard && (
        <li className="mb-4 pr-10 pl-5 pt-5">
